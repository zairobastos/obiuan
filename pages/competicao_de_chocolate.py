...
with col1:
    ...
    with col22:
        st.header("Competição de chocolate")

    st.text("Carlos e Paula acabaram de ganhar um saco com bolinhas de chocolate. Como sabem que vão comer tudo muito rápido inventaram uma brincadeira:")
    st.markdown("&emsp; • Eles vão comer de forma alternada, um depois o outro, sendo que sempre a Paula começa.")
    st.markdown("&emsp; • A cada vez, só se pode comer de 1 a M bolinhas, sendo o M decidido pela mãe de Paula, de forma que não engasguem com o chocolate.")
    st.markdown("&emsp; • Se um comeu K bolinhas em sua vez, o próximo não pode comer o mesmo tanto, tendo que comer um número de bolinhas distinto.")
    st.markdown("&emsp; • Quem não puder mais jogar de maneira válida perde.")
    st.text("Um exemplo de partida para M = 5 e 20 bolinhas, onde Carlos ganhou:")

    col14, col24, col34 = st.columns([1, 1, 1])

    with col24:
        st.image("./images/competicao_chocolate.png",width=700)

    st.text("Observe que no final Carlos não poderia comer 2 bolinhas para ganhar, pois seria o mesmo que Paula comeu na vez anterior. Mas Paula também não pôde comer a última bolinha, pois Carlos havia comido apenas uma na rodada anterior, assim Paula ficou sem opção de jogada e perdeu.")
    st.text("Ambos são muito espertos e jogam de maneira ótima, de forma que se existe para um deles uma sequência de jogadas que garante a vitória independente da jogada do outro, essa pessoa jogará dessa forma.")

    st.subheader("Tarefa")
    st.text("Sua tarefa é determinar quem vai ganhar a brincadeira, se ambos jogam de forma ótima.")
    ...
    st.text("A entrada consiste de uma linha contendo dois inteiros N (2 ≤ N ≤ 10⁶) e M (2 ≤ M ≤ 10³), sendo N o número de bolinhas de chocolate e M o número de bolinhas permitidas por vez.")

    st.subheader("Saída")
    st.text("Seu programa deve imprimir, na saída padrão, uma linha, contendo o nome do vencedor, como exemplificado abaixo.")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos de teste que totaliza 50 pontos, N ≤ 50 e M ≤ 5.")
    st.markdown("&emsp; • Em um conjunto de casos de teste que totaliza 80 pontos, N ≤ 10⁴ e M ≤ 100.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Competição de chocolate"
            dados.rota_questao = "competicao_de_chocolate"
