...
with col1:
    ...
    with col22:
        st.header("Fotografia")

    st.text("Chegou o final do ano, e os alunos resolveram dar um presente para a profa. Vilma: um quadro com a fotografia dos alunos classe. João está pesquisando os preços de molduras em um site da internet, e precisa de sua ajuda. Ele quer encontrar uma moldura tal que")
    st.markdown("&emsp; • a fotografia seja inteiramente contida dentro na moldura; e")
    st.markdown("&emsp; • a área da moldura não ocupada pela fotografia seja a menor possível; e")
    st.markdown("&emsp; • uma moldura pode ser rotacionada para acomodar a fotografia, mas a fotografia deve ser acomodada de forma que seus lados sejam paralelos aos lados da moldura.")
    st.text("Dada as dimensões da fotografia e das molduras disponíveis, escreva um programa para ajudar João a escolher a melhor moldura, segundo os critérios acima.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém dois inteiros A e L indicando respectivamente a altura e largura da fotografia, em centímetros. A segunda linha da entrada contém um inteiro N, a quantidade de molduras disponíveis. As molduras são identificadas por números de 1 a N. Cada uma das N linhas seguintes contém dois inteiros Xi e Yi indicando as dimensões em centímetros da moldura de número i, para 1 ≤ i ≤ N.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha na saída, contendo um único número inteiro, o identificador da melhor moldura de acordo com os critérios acima. Se houver mais de uma moldura que satisfaz os critérios, o programa deve produzir o menor identificador entre as mulduras que satisfazem os critérios. Se nenhuma moldura satisfaz os critérios, a linha deve conter -1.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ A ≤ 100")
    st.markdown("&emsp; • 1 ≤ L ≤ 100")
    st.markdown("&emsp; • 1 ≤ N ≤ 100")
    st.markdown("&emsp; • 1 ≤ Xi ≤ 200 para 1 ≤ i ≤ N")
    st.markdown("&emsp; • 1 ≤ Yi ≤ 200 para 1 ≤ i ≤ N")
    st.markdown("&emsp; • não há duas molduras com as mesmas dimensões")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Fotografia"
            dados.rota_questao = "fotografia"
