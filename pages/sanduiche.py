...
with col1:
    ...
    st.text("""Uma nova lanchonete abriu na cidade, prometendo um menu com a maior variedade de sanduíches da região. A cada dia o Chef de cozinha compra N ingredientes distintos e prepara o menu usando esses N ingredientes. Infelizmente não é possível ter sanduíches com qualquer combinação de ingredientes: a cada dia o Chef determina que M pares de ingredientes não podem ser utilizados no mesmo sanduíche, porque ele considera que esses ingredientes "não combinam".""")
    st.text("Por exemplo, suponha que num determinado dia N é igual a quatro e os ingrediantes são queijo, presunto, goiabada e azeitona, e M é igual a dois: os pares (goiabada, presunto) e (azeitona, goiabada) não podem ser utilizados no mesmo sanduíche. Nesse dia, alguns dos sanduíches que podem ser feitos são:")
    st.markdown("&emsp; • presunto, queijo")
    st.markdown("&emsp; • azeitona")
    st.markdown("&emsp; • presunto, azeitona, queijo")
    st.markdown("&emsp; • goiabada, queijo")
    st.text("Alguns dos sanduíches que não podem ser feitos são:")
    st.markdown("&emsp; • presunto, queijo, goiabada")
    st.markdown("&emsp; • azeitona, goiabada")
    st.markdown("&emsp; • goiabada, presunto, azeitona")
    st.text("Dados os N ingredientes e os M pares de ingredientes que não combinam, sua tarefa é determinar qual o máximo número de sanduíches diferentes que podem ser feitos. Dois sanduíches A e B são considerados diferentes se A contém um ingrediente X que não está presente em B ou se B contém um ingrediente Y que não está presente em A. Um sanduíche deve conter ao menos um ingrediente.")
    ...
    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, o número de sanduíches diferentes que podem ser feitos.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 20")
    st.markdown("&emsp; • 0 ≤ M ≤ 400")
    st.markdown("&emsp; • 1 ≤ X ≤ N")
    st.markdown("&emsp; • 1 ≤ X < Y")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 10 pontos, N ≤ 5.")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo outros 40 pontos, N ≤ 10.")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo outros 50 pontos, nenhuma restrição adicional.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Sanduíche"
            dados.rota_questao = "sanduiche"
