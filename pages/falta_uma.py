...
with col1:
    ...
    st.text("Carolina tem um jogo de tabuleiro que possui 24 cartas contendo, cada uma, uma permutação dos quatro primeiros números naturais. (Cartas distintas contêm permutações distintas.) Lembre-se de que a quantidade de permutações de quatro números é 4!, que é igual a 24. Só que ela contou e encontrou apenas 23 cartas. Está faltando uma! Dê uma olhada nessa lista embaralhada de 23 cartas. Qual está faltando?")
   
    col14, col24, col34 = st.columns([1.1, 1, 1])

    with col24:
        st.image("./images/falta_uma.png",width=300)

    st.text("PAgora suponha que o jogo tenha um baralho de N! cartas, com todas as permutações possíveis dos N primeiros naturais. Neste problema, dado N e uma lista com N!-1 cartas, seu programa deve imprimir a carta que está faltando.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um inteiro N. As N!-1 linhas seguintes contêm, cada uma, N naturais. Cada linha representa uma permutação distinta dos N primeiros naturais.")

    st.subheader("Saída")
    st.text("Seu programa deve imprimir uma única linha, contendo N naturais representando a permutação que está faltando na entrada.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 2 ≤ N ≤ 8.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Falta uma"
            dados.rota_questao = "falta_uma"
