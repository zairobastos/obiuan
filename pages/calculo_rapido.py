...
with col1:
    ...
    st.text("Algumas pessoas conseguem fazer cálculos matemáticos com uma velocidade impressionante. Laurinha tem essa habilidade! Um cálculo que ela consegue fazer muito rapidamente é, dados três números inteiros S, A, e B, determinar quantos números do intervalo [A, B] têm a soma de seus dígitos igual a S.")
    st.text("Por exemplo, se S = 3, A = 10 e B = 30, então a reposta é 3, pois existem três números no intervalo [10,30] cuja soma dos dígitos é igual a três: 12, 21 e 30.")
    st.text("Sua tarefa é escrever um programa de computador para, dados os três números, tentar calcular a resposta mais rapidamente do que Laurinha consegue.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um número inteiro S, o valor da soma dos dígitos. A segunda e a terceira linhas contêm respectivamente os inteiros A e B.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo um único inteiro, quantos números no intervalo dado têm a soma de dígitos indicada.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ S ≤ 36")
    st.markdown("&emsp; • 1 ≤ A ≤ 10 000")
    st.markdown("&emsp; • 1 ≤ B ≤ 10 000")
    st.markdown("&emsp; • A ≤ B")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Cálculo Rápido"
            dados.rota_questao = "calculo_rapido"
