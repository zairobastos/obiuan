...
with col1:
    ...
    with col22:
        st.header("Passatempo")

    st.text("Desafios de lógica e matemática são um ótimo passatempo, atraindo um grande número de praticantes, e um mercado que envolve aplicativos para celular, atividades on-line, revistas especializadas e até mesmo cursos na internet para melhorar o desempenho!")
    st.text("Neste problema sua tarefa é escrever um programa que resolva um novo passatempo, mostrado na figura abaixo. O passatempo é composto por um quadriculado com letras dentro de cada célula e números ao lado de cada linha ou coluna do quadriculado. As letras dentro de cada célula representam variáveis, e os números representam as somas dos valores das variáveis em cada linha ou coluna.")
    
    col14, col24, col34 = st.columns([1, 1, 1])

    with col24:
        st.image("./images/passatempo.png", width=300)

    st.text("O objetivo desse passatempo é determinar o valor de cada variável de modo a satisfazer as somas das linhas e colunas mostradas. Para permitir que um número maior de pessoas consiga resolver o passatempo, ele tem uma propriedade que facilita a sua solução: sempre é possível encontrar uma linha ou coluna em que há apenas uma variável cujo valor ainda é desconhecido. Assim, uma possível maneira de resolver o problema é, a cada passo da solução, encontrar o valor de uma variável.")
    st.text("Sua tarefa é, dado um passatempo, determinar os valores das variáveis que o solucionam.")
    ...
    st.subheader("Saída")
    st.text("Seu programa deve produzir uma linha para cada variável do passatempo, contendo o nome da variável e o seu valor inteiro. As variáveis devem ser escritas em ordem alfabética crescente, ou seja, respeitando a ordem aa, ab, …, az, ba, bb, …, za, zb, …, zz.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ L ≤ 100")
    st.markdown("&emsp; • 2 ≤ C ≤ 100")
    st.markdown("&emsp; • -10⁸ ≤ S ≤ 10⁸")
    st.markdown("&emsp; • -10⁸ ≤ Xi ≤ 10⁸")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Passatempo"
            dados.rota_questao = "passatempo"
