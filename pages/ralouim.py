...
with col1:
    ...
    st.text("Para a tradicional festa infantil de Ralouim, o rei da Nlogônia instalou tendas de distribuição de guloseimas no seu extenso Jardim Real, onde está também situado o Palácio Real.")
    st.text("Cada tenda tem uma quantidade ilimitada de guloseimas. As crianças devem sair do Palácio Real e visitar as tendas para ganhar guloseimas, mas o Rei estabeleceu algumas regras:")
    st.markdown("&emsp; • a cada visita a uma tenda, a criança ganha exatamente uma guloseima.")
    st.markdown("&emsp; • uma tenda pode ser visitada mais de uma vez pela mesma criança, desde que as visitas não sejam consecutivas (ou seja, uma imediatamente após a outra).")
    st.markdown("""&emsp; • as distâncias que uma criança percorre para chegar à "próxima tenda" devem ser estritamente decrescentes. Ou seja, a distância que a criança percorre do Palácio até a primeira tenda que a criança visita deve ser maior do que a distância que a criança percorre entre a primeira tenda e a segunda tenda, que por sua vez deve ser maior do que a distância que a criança percorre entre a segunda tenda e a terceira tenda, e assim por diante.""")
    st.text("Pedrinho percebeu que se planejar direito suas visitas, pode ganhar muitas guloseimas! Escreva um programa para ajudar Pedrinho a ganhar o maior número possível de guloseimas no Ralouim.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um inteiro N, o número de tendas. Cada uma das N linhas seguintes contém dois inteiros X e Y, as coordenadas de uma tenda no Jardim Real. A localização do Palácio Real é (0,0) e não existe tenda com essas coordenadas, todas as tendas têm localizações distintas.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo um único inteiro, o maior número de guloseimas que Pedrinho pode ganhar.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 2000")
    st.markdown("&emsp; • -10000 ≤ X ≤ 10000")
    st.markdown("&emsp; • -10000 ≤ Y ≤ 10000")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 20 pontos, 1 ≤ N ≤ 50.")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 40 pontos adicionais, 1 ≤ N ≤ 200.")
    st.markdown("&emsp; • Para um conjunto de casos de testess valendo 40 pontos adicionais, nenhuma restrição adicional.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Ralouim"
            dados.rota_questao = "ralouim"
