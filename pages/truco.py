...
with col1:
    ...
    st.text("Truco é um jogo de cartas que pode ser jogado por duas ou mais pessoas. Existem diversas variações: o Truco Cego ou Truco Espanhol (popular no sul do Brasil, Argentina, Uruguai e outros países), o Truco Paulista, Capixaba ou Mineiro (variações populares no Brasil), o Truco Índio e o Truco Eteviano. Em geral, é urna disputa de três rodadas ( “melhor de três” ) para ver quem tem as cartas mais “fortes” (de valor simbólico mais alto).")
    st.text("""Adalberto e Bernardete estão jogando uma variação de truco com 40 cartas (foram retirados do baralho todas as cartas de valor 8, 9 e 10, além dos coringas) , e o valor simbólico independente do naipe da carta. A ordem de valor simbólico das cartas nessa variação de truco é mostrada abaixo, ordenada da mais “fraca” (mais à esquerda) para a mais "forte" (mais à direita)""")
    
    col14, col24, col34 = st.columns([1.5, 1, 1])
    with col24:
        st.text("4 5 6 7 Q J K A 2 3")

    st.text("Cada partida é disputada em três rodadas. A cada rodada, os jogadores escolhem uma das cartas para mostrar, e vence aquele que tiver a carta com o maior valor simbólico. Em caso de empate (ou seja, os dois apresentarem cartas com os mesmos valores simbólicos), Adalberto vence, pois é mais velho que Bernardete. Vence a partida aquele que vencer o maior número de rodadas. Depois de algumas partidas, Adalberto e Bernardete estão com dificuldades para saber quem venceu mais partidas, e pediram a sua ajuda.")
    st.text("Sua tarefa é escrever um programa que calcule o número de partidas que cada um dos competidores (Adalberto e Bernarrlete) venceram.")

    st.subheader("Entrada")
    st.text("A primeira linha da saída possui um inteiro N que indica o número de partidas disputadas entre Adalberto e Bernardete. As N linhas seguintes contem cada uma seis inteiros, A1, A2, A3, B1, B2 e B3, que correspondem às três cartas apresentadas por Adalberto nas rodadas 1, 2 e 3 daquela partida, seguidas pelas três cartas apresentadas por Bernardete nas rodadas 1, 2 e 3 da mesma partida. Na entrada, o número 1 representa o As (A), 11 representa o Valete (J), 12 representa a Dama (Q) e 13 representa o Rei (K).")

    st.subheader("Saída")
    st.text("Seu programa deve imprimir uma única linha, que contém os números de partidas vencidas por Adalberto e por Bernardete, nessa ordem.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 1 000 000")
    st.markdown("&emsp; • 1 ≤ Ai ≤ 13")
