...
with col1:
    ...
    st.text("Uma fábrica instalou um elevador composto de duas cabines ligadas por uma roldana, como na figura. Quando uma cabine sobe, a outra desce. No primeiro andar da fábrica existem algumas caixas de pesos diversos e precisamos levar todas as caixas para o segundo andar, usando o elevador. Apenas uma caixa pode ser colocada por vez dentro de uma cabine. Além disso, existe uma restrição de segurança importante: durante uma viagem do elevador, a diferença de peso entre as cabines pode ser no máximo de 8 unidades. De forma mais rigorosa, P-Q ≤ 8, onde P é o peso da cabine mais pesada e Q, o peso da cabine mais leve. O gerente da fábrica não está preocupado com o número de viagens que o elevador vai fazer. Ele apenas precisa saber se é possível ou não levar todas as caixas para o segundo andar. No exemplo da figura, podemos levar todas as três caixas usando a seguinte sequência de seis viagens do elevador:")
    col14, col24, col34 = st.columns([1.2, 1, 1])

    with col24:
        st.image("./images/elevador.png",width=300)

    st.markdown("&emsp; • Sobe a caixa de peso 4, desce a outra cabine vazia; (diferença de 4)")
    st.markdown("&emsp; • Sobe a caixa de peso 10, desce a caixa de peso 4; (diferença de 6)")
    st.markdown("&emsp; • Sobe a caixa de peso 15, desce a caixa de peso 10; (diferença de 5)")
    st.markdown("&emsp; • Sobe a caixa de peso 4, desce a outra cabine vazia; (diferença de 4)")
    st.markdown("&emsp; • Sobe a caixa de peso 10, desce a caixa de peso 4; (diferença de 6)")
    st.markdown("&emsp; • Sobe a caixa de peso 4, desce a outra cabine vazia. (diferença de 4)")
    st.text("Dados os pesos de N caixas no primeiro andar, seu programa deve determinar se é possível ou não levar todas as N caixas para o segundo andar.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um inteiro N indicando o número de caixas. A segunda linha da entrada contém N inteiros representando os pesos das caixas.")

    st.subheader("Saída")
    st.text("Imprima uma linha na saída. A linha deve conter o caracter S caso seja possível, ou N caso não seja possível levar todas as caixas até o segundo andar da fábrica.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 10⁴")
    st.markdown("&emsp; • O peso das caixas está entre 1 e 10⁵, inclusive.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Elevador"
            dados.rota_questao = "elevador"
