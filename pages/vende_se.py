...
with col1:
    ...
    st.text("A Otacílio Busílis Imóveis (OBI) é a maior imobiliária de Nlogópolis, especializada no aluguel de prédios comerciais; todas as suas propriedades se localizam na Avenida Doutor Otacílio Busílis, assim chamada em homenagem ao fundador da OBI.")
    st.text("Devido à crise econômica mundial, a OBI precisa vender K de seus imóveis para levantar capital de giro. Dr.Otacílio quer que os prédios restantes após a venda sejam o mais próximos possível --- ou seja, a distância entre o primeiro e o último prédios restantes deve ser a menor possível.")
    st.text("Infelizmente, a OBI é proprietária de tantos prédios que o Dr.Otacílio não sabe quais prédios ele deve vender; ele lhe contratou para que você escreva um programa que determina qual é a mínima distância possível entre o primeiro e o último prédios da OBI na avenida, após a venda de K prédios.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém os inteiros N e K, indicando, respectivamente, quantos prédios a OBI possui, e quantos prédios ela pretende vender. A linha seguinte contém N inteiros X_i, indicando a distância de cada um dos N prédios ao início da avenida, em metros.")

    st.subheader("Saída")
    st.text("A saída deve conter um único inteiro indicando a menor distância possível entre o primeiro e o último prédio possuídos pela OBI após a venda.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 3 ≤ N ≤ 10⁵")
    st.markdown("&emsp; • N - K ≥ 2")
    st.markdown("&emsp; • 1 ≤ X_i ≤ 10⁶")
    st.markdown("&emsp; • todos os X_i são distintos")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos de teste que totaliza 30 pontos, N ≤ 10.")
    st.markdown("&emsp; • Em um conjunto de casos de teste que totaliza 70 pontos, N ≤ 2000.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Vende-se"
            dados.rota_questao = "vende_se"
