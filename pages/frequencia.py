...
with col1:
    ...
    st.text("Byteland é uma cidade bastante conhecida por propor variados desafios aos seus habitantes. Recentemente, o prefeito de Byteland, Joãozinho, decidiu propor um desafio que ele gosta de chamar de Tabuleiro da Frequência.")
    st.text("""A brincadeira ocorre da seguinte forma. Inicialmente, um tabuleiro com dimensões Nx N é dado contendo apenas 0"s. Depois disso, Q operações são propostas, podendo ser de 4 tipos:""")
    st.markdown("&emsp; • 1 X R: Atribuir o valor R a todos os números da linha X;")
    st.markdown("&emsp; • 2 X R: Atribuir o valor R a todos os números da coluna X;")
    st.markdown("&emsp; • 3 X: Imprimir o valor mais frequente na linha X;")
    st.markdown("&emsp; • 4 X: Imprimir o valor mais frequente da coluna X.")
    st.text("Joãozinho é muito bom com computadores, mas também é bastante preguiçoso. Sabendo que você é um dos melhores programadores do mundo, ele decidiu pedir sua ajuda para resolver este problema.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada é composta por dois inteiros N e Q, representando, respectivamente, o tamanho do tabuleiro e a quantidade de operações. As próximas Q linhas da entrada vão conter as Q operações. O primeiro inteiro de cada linha vai indicar o tipo da operação. Caso seja 1 ou 2, será seguido por mais dois inteiros X e R. Caso seja 3 ou 4, será seguido por apenas mais um inteiro X.")

    st.subheader("Saída")
    st.text("Para cada operação do tipo 3 ou 4, seu programa deve produzir uma linha, contendo o valor da resposta correspondente. Se uma linha ou coluna tiver dois ou mais valores que se repetem o mesmo número de vezes, você deve imprimir o maior deles. Por exemplo, se uma linha tem os valores [5,7,7,2,5,2,1,3], tanto o 2, 5 e 7 se repetem duas vezes, então a resposta será 7, pois é o maior deles.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N, Q ≤ 10⁵")
    st.markdown("&emsp; • 1 ≤ X ≤ N")
    st.markdown("&emsp; • 0 ≤ R ≤ 50")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos de teste equivalente a 30 pontos, N ≤ 10³.")
    st.markdown("&emsp; • Em um conjunto de casos de teste equivalente a 20 pontos, apenas as operações 2 e 3 serão usadas.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Frequência"
            dados.rota_questao = "frequencia"
