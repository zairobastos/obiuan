...
with col1:
    st.title("OBI-UAN")
    col12, col22, col32 = st.columns([1.2, 1, 1])

    with col22:
        st.header("Cifra da Nlogônia")

    st.text("""O rei da Nlogônia ordenou que todos os documentos importantes sejam "cifrados", para que apenas quem tem conhecimento da cifra possa lê-los (cifrar um documento significa alterar o original modificando as letras de acordo com algum algoritmo de cifra).""")
    st.text("O rei decretou que o seguinte algotimo deve ser usado para cifrar os documentos:")
    st.markdown("&emsp; • Cada consoante deve ser substituída por exatamente três letras, na seguinte ordem:")
    st.markdown("&emsp;&emsp; 1. a própria consoante (vamos chamar de consoante original);")
    st.markdown("&emsp;&emsp; 2. a vogal mais próxima da consoante original no alfabeto, com a regra adicional de que se a consoante original está à mesma distância de duas vogais, então a vogal mais próxima do início do alfabeto é usada. Por exemplo, se a consoante original é d, a vogal que deve ser usada é e, pois esta é a vogal mais próxima; se a consoante original é c, a vogal que deve ser utilizada é a, porque c está à mesma distância de a e e, e a é mais próxima do início do alfabeto.")
    st.markdown("&emsp;&emsp; 3. a consoante que segue a consoante original, na ordem do início ao fim do alfabeto. Por exemplo, se a consoante original é d, a consoante a ser utilizada é f. No caso de a consoante original ser z, deve ser utilizada a própria letra z.")
    st.markdown("&emsp; • As vogais não são modificadas.")
    st.text("Nesta tarefa, o alfabeto é")

    col14, col24, col34 = st.columns([1.2, 1, 1])

    with col24:
        st.text("a b c d e f g h i j k l m n o p q r s t u v x z")

    st.text("e as vogais são")

    col15, col25, col35 = st.columns([1.7, 1, 1])

    with col25:
        st.text("a e i o u")

    st.text("""Por exemplo, a cifra da palavra "ter" é "tuveros" (tuv-e-ros) e a cifra da palavra "paz" é "poqazuz" (poq-a-zuz).""")
    st.text("O rei da Nlogônia procura por alguém que saiba escrever um programa que produza a cifra de uma palavra dada. Você pode ajudá-lo?")

    st.subheader("Entrada")
    st.text("A primeira e única linha da entrada contém uma palavra P formada por letras minúsculas sem acentuação.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo a palavra cifrada.")

    st.subheader("Restrições")
    st.markdown("&emsp; • A palavra P tem no mínimo uma e no máximo 30 letras, todas minúsculas e sem acentuação.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Cifra da Nlogônia"
            dados.rota_questao = "cifra_da_nlogonia"
