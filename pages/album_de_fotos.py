...
with col1:
    st.title("OBI-UAN")
    col12, col22, col32 = st.columns([1.3, 1, 1])

    with col22:
        st.header("Álbum de fotos")

    st.text("Clara está organizando as fotos da sua última viagem num álbum de fotos. Como ela tem muitas fotos, para economizar páginas do álbum ela quer colar duas fotos por página do álbum.")
    st.text("Como as fotos são retangulares, as fotos podem ser coladas giradas (mas sempre com lados paralelos aos da página do álbum, para preservar o equilíbrio estético do álbum), mas elas devem sempre ficar inteiramente contidas no interior da página, e não devem se sobrepor.")
    st.text("Em geral, das muitas formas de posicionar as fotos do álbum só algumas (ou nenhuma) satisfazem estas restrições, então pode ser difícil decidir se é possível colar as duas fotos em uma mesma página do álbum, e por isso Clara pediu a sua ajuda para escrever um programa que, dadas as dimensões da página e das fotos, decide se é possível colar as fotos na página.")
    st.text("Por exemplo, cada página pode ser 5×7, e duas fotos são 3×4. Nesse caso, é possível colar as duas fotos:")

    st.image("./images/album.png",width=300)

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém dois inteiros X e Y , indicando a largura e a altura da página do álbum. Cada uma das duas linhas seguintes contém dois inteiros L e H, indicando a largura e a altura das fotos.")

    st.subheader("Saída")
    st.text("""Imprima uma única linha, contendo um único caractere: "S", se é possível colar as duas fotos na página do álbum, e "N", caso contrário.""")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ X, Y ≤ 1000")
    st.markdown("&emsp; • 1 ≤ L, H ≤ 1000")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Álbum de fotos"
            dados.rota_questao = "album_de_fotos"
