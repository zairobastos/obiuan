...
with col1:
    ...
    with col22:
        st.header("Casamento de inteiros")

    st.text("Vamos definir a operação de casamento de dois números inteiros A e B da seguinte forma:")
    st.markdown("&emsp; • inicialmente fazemos A e B terem o mesmo número de dígitos, adicionando zeros à esquerda conforme necessário;")
    st.markdown("&emsp; • então cada dígito de A (do menos significativo ao mais significativo) é comparado com o dígito correspondente de B, e o dígito de menor valor é eliminado do número a que pertence (se os dígitos são iguais nenhum é eliminado).")
    st.markdown("&emsp; • o resultado da operação de casamento é o par de números inteiros formados pelos dígitos remanescentes de A e B. No caso de não haver digito remanescente para um dos números, o resultado para esse número é -1.")
    st.text("Por exemplo, considere o casamento de 69961 com 487920:")
    st.text("O resultado do casamento é o par de números 489 e 9961.")
    st.text("Dados dois números inteiros, sua tarefa é determinar o resultado do casamento desses dois números.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um número inteiro A, a segunda linha contém um número inteiro B.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo os dois números inteiros produzidos pelo casamento dos números dados, em ordem não decrescente.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ A ≤ 10⁹")
    st.markdown("&emsp; • 1 ≤ B ≤ 10⁹")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 22 pontos, 100 ≤ A ≤ 999 e 100 ≤ B ≤ 999.")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo outros 78 pontos, nenhuma restrição adicional.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Casamento de inteiros"
            dados.rota_questao = "casamento_de_inteiros"
