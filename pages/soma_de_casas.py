...
with col1:
    ...
    with col22:
        st.header("Soma de casas")

    st.text("Joãozinho mora em uma rua que tem N casas. Marquinhos é o melhor amigo dele, mas sempre gosta de pregar peças em Joãozinho. Desta vez, ele pegou os dois brinquedos prediletos de Joãozinho e os escondeu em duas casas distintas da rua. Em compensação, Marquinhos deu uma dica importante para Joãozinho:")
    st.text("A soma dos números das casas em que escondi teus brinquedos é igual a K. Além disso, escolhi as casas de tal forma que não existe outro par de casas cuja soma tenha esse mesmo valor.")
    st.text("Sabendo disto, encontre qual é o par de casas em que se encontram os brinquedos de Joãozinho. Para auxiliar seu amigo, Marquinhos entregou a Joãozinho uma lista com o número das casas já em ordem crescente (isto é, do menor para o maior número).")

    st.subheader("Entrada")
    st.text("A primeira primeira linha da entrada contém um número inteiro N, que representa o número de casas que existem na rua. Cada uma das N linhas seguintes contém um número inteiro, representando o número de uma casa. Note que esses N números estão ordenados, do menor para o maior. A última linha da entrada contém um inteiro K, que é a soma dos números das duas casas onde os brinquedos estão escondidos.")

    st.subheader("Saída")
    st.text("Se programa deve imprimir uma única linha, contendo dois inteiros, A e B, A < B, que representam os números das casas em que estão escondidos os brinquedos. Os dois números devem ser separados por um espaço em branco.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 2 ≤ N ≤ 10⁵")
    st.markdown("&emsp; • Para cada casa Ci, 0 ≤ Ci ≤ 109, i = 1, 2, ... , N")
    st.markdown("&emsp; • Os números das casas estão em ordem crescente, do menor para o maior número, e casas distintas têm números distintos.")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos que totaliza 30 pontos, N ≤ 10³.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Soma de casas"
            dados.rota_questao = "soma_de_casas"
