...
with col1:
    ...
    st.text("João dispõe de dois pedaços retangulares de tecido, e quer usá-los para fazer um lençol, também retangular, de dimensões A x B. Se necessário, os dois pedaços retangulares podem ser unidos por uma costura, mas João quer que a costura seja paralela aos lados dos retângulos. Os cortes, se necessários, também devem ser paralelos aos lados dos retângulos. Dadas as dimensões dos pedaços de tecido e do lençol, escreva um programa que determina se é possível João fazer o lençol com as dimensões desejadas.")

    st.subheader("Entrada")
    st.text("A entrada contém uma única linha, com seis inteiros A1, B1, A2, B2, A E B, representando, respectivamente, as dimensões dos dois retângulos disponíveis, e as dimensões do retângulo desejado.")

    st.subheader("Saída")
    st.text("Seu programa deve imprimir uma única linha contendo um caractere S se é possível fazer o lençol, e N caso contrário.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ A1, B1, A2, B2, A, B ≤ 10⁶")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Lençol"
            dados.rota_questao = "lencol"
