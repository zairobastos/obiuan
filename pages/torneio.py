...
with col1:
    ...
    st.text("Juquinha foi convidado para participar do prestigiado torneio de tênis de Rolando Barros, na Nlogônia. O torneio é composto de N rodadas no estilo mata-mata: todo jogador que perde uma partida é eliminado do torneio, e o vencedor desta partida avança para a próxima rodada. Como o número de jogadores ativos cai pela metade a cada rodada, é necessário que o número de jogadores participantes seja uma potência de 2.")
    
    col14, col24, col34 = st.columns([1, 1, 1])

    with col24:
        st.image("./images/torneio.png", width=600)

    st.text("Os jogadores são inicialmente dispostos na chave por sorteio. Em uma disposição é atribuido a cada jogador um valor de 1 a 2^N, que corresponde a sua posição na chave do torneio. Jogadores vencedores avançam para a direita da chave, e disputam com o vencedor da sub-chave vizinha. Na imagem acima, caso os jogadores das posições 1 e 3 vençam suas partidas na primeira rodada, estes se enfrentarão na segunda rodada.")
    st.text("Juquinha não quer perder a chance de tornar-se um jogador mundialmente famoso, e para isso contratou você para ajudá-lo em suas análises estatísticas. Ele atribuiu a cada jogador um coeficiente de habilidade H_i, e sabe que se dois jogadores disputarem uma partida, aquele com maior coeficiente de habilidade certamente será o vencedor. Seu papel é calcular quantas disposições iniciais dos jogadores forçam Juquinha perder na K-ésima rodada (ou vencer o torneio, caso K = N+1). Duas disposições são consideradas distintas se para algum jogador foi atribuido um valor diferente nas duas disposições.")

    st.subheader("Entrada")
    st.text("A primeira linha contém dois inteiros N e K. Cada uma das próximas 2^N linhas seguintes contêm um único inteiro representando o coeficiente de habilidade de um jogador. O coeficiente de Juquinha é representado pelo primeiro desses inteiros.")

    st.subheader("Saída")
    st.text("Seu programa deve imprimir uma única linha contendo um único inteiro indicando o número de disposições iniciais que forçam Juquinha a perder na K-ésima rodada (ou ganhar o torneio, se K = N+1). Como este número pode ser muito grande, imprima o resto que este número deixa quando dividido por 1.000.000.007 (10⁹ + 7).")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 16")
    st.markdown("&emsp; • 1 ≤ K ≤ N+1")
    st.markdown("&emsp; • 0 ≤ coeficiente de habilidade de um jogador ≤ 10⁹")
    st.markdown("&emsp; • Não existem dois jogadores diferentes com a mesma habilidade.")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos de testes que totaliza 30 pontos, N ≤ 3")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Torneio"
            dados.rota_questao = "torneio"
