...
with col1:
    ...
    st.text("Otávio tem dois irmãos, um mais velho (Orlando) e um mais novo do que ele (Oscar). As idades dos três irmãos formam uma progressão aritmética: a diferença de idade dos dois irmãos mais novos (Otávio e Oscar) é igual à diferença de idade dos dois irmãos mais velhos (Orlando e Otávio).")
    st.text("Dadas as idades de Otávio e de seu irmão mais novo, escreva um programa para determinar a idade do irmão mais velho.")
    
    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um inteiro N, a idade do irmão mais novo de Otávio. A segunda linha contém um inteiro M, a idade de Otávio.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir na saída uma única linha, contendo um único número inteiro, a idade do irmão mais velho de Otávio.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 40")
    st.markdown("&emsp; • N ≤ M ≤ 40")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Irmãos"
            dados.rota_questao = "irmaos"
