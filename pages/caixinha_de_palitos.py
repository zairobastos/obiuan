...
with col1:
    ...
    with col22:
        st.header("Caixinha de Palitos")

    st.text("A caixinha contém N palitos de picolé, que precisam ser divididos entre os amigos Renato, Gustavo e Bruno, para um trabalho escolar. Cada amigo deve ganhar pelo menos 1 (um) palito. O professor vai determinar um número M máximo de palitos que cada um pode ganhar. Nesta tarefa, dados N e M, seu programa deve calcular quantas maneiras distintas existem de se dividir todos os N palitos entre os três amigos. Por exemplo, para N=100: se M=15, então há zero maneiras de se dividir, pois a soma dos números de palitos de Renato, Gustavo e Bruno seria no máximo 45, só que precisa ser sempre N; mas se M=34, aí veja que haveria 6 maneiras distintas:")
    
    col14, col24, col34 = st.columns([1, 1, 1])
    with col24:
        st.image("./images/caixinha.png",width=300)

    st.subheader("Entrada")
    st.text("A entrada é composta por apenas uma linha com dois números naturais N e M, indicando, respectivamente, o número de palitos na caixinha e o número máximo que cada amigo pode ganhar.")

    st.subheader("Saída")
    st.text("Seu programa deve escrever uma única linha na saída, contendo um único número natural: quantas maneiras distintas existem de se dividir os N palitos entre os três amigos.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 3 ≤ N ≤ 100000, 1 ≤ M ≤ N;")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos de teste somando 20 pontos, N ≤ 300")
    st.markdown("&emsp; • Em um conjunto de casos de teste somando 50 pontos, N ≤ 30000")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Caixinha de Palitos"
            dados.rota_questao = "caixinha_de_palitos"
