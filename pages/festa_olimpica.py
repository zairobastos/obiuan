...
with col1:
    ...
    with col22:
        st.header("Festa olímpica")

    st.text("Os atletas da Nlogônia obtiveram o melhor resultado do país em olimpíadas, e para comemorar o rei decidiu dar uma grande festa no Palácio Real. Todos os atletas foram convidados, mas o rei quer também convidar alguns de seus súditos.")
    st.text("Como não é possível convidar todos os súditos, o rei determinou que a seguinte Lei seja utilizada para calcular a lista de convidados:")
    st.markdown("***Lei Especial Sobre Comemoração das Olimpíadas***")
    st.text("Por ordem de Sua Majestade, fiquem todos sabendo que:")
    st.markdown("&emsp; • Os N súditos de Nlogônia serão numerados 1, 2, 3, …, N e uma lista ordenada será criada com os números dos súditos. A primeira posição da lista será 1.")
    st.markdown("&emsp; • Um número M de turnos serão então executados; em cada turno i, será sorteado um número Ti que será usado para remover súditos da lista, da seguinte forma: no turno i, devem ser removidos da lista todos os súditos que ainda continuam na lista e que ocupam posições que são múltiplas de Ti; ou seja, devem ser removidos os súditos que estão nas posições (Ti, 2Ti, 3Ti, …) da lista corrente. Ao final do turno, para não haver posições vazias na lista (cujos súditos foram removidos) a lista é reagrupada, mantendo-se a mesma ordem relativa, e contendo apenas os números dos súditos remanescentes.")
    st.markdown("&emsp; • Os súditos que permanecerem na lista ao final dos M turnos serão convidados para a grande festa de comemoração do resultado das olimpíadas.")
    st.text("Dados o número de súditos e os números sorteados em cada turno, sua tarefa é determinar os súditos que serão convidados de acordo com a Lei Especial.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um número inteiro N, o número de súditos de Nlogônia. A segunda linha contém um inteiro M, o número de turnos. Cada uma das M linhas seguintes contém um inteiro Ti, o número que foi sorteado para o turno i.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir a lista de convidados de acordo com a Lei Especial, com uma linha para cada convidado, cada linha contendo somente o número de um convidado. Como a lista total dos convidados pode ser muito grande, o rei ordenou que, caso o número de convidados seja maior que 10.000, você deve listar apenas os 10.000 primeiros (ou seja, os com menores números) convidados.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 2 ≤ N ≤ 1 000 000 000")
    st.markdown("&emsp; • 1 ≤ M ≤ 5 000")
    st.markdown("&emsp; • 2 ≤ Ti ≤ 100 000 para 1 ≤ i ≤ M")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 17 pontos, N ≤ 100 e M ≤ 10.")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo outros 22 pontos, N ≤ 400 000 e M ≤ 5 000.")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo outros 21 pontos, Ti = 2 para 1 ≤ i ≤ M.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Festa olímpica"
            dados.rota_questao = "festa_olimpica"
