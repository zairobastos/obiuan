...
with col1:
    ...
    with col22:
        st.header("Idade de Camila")

    st.text("Cibele, Camila e Celeste são três irmãs inseparáveis. Estão sempre juntas e adoram fazer esportes, ler, cozinhar, jogar no computador... Agora estão aprendendo a programar computadores para desenvolverem seus próprios jogos.")
    st.text("Mas nada disso interessa para esta tarefa: estamos interessados apenas nas suas idades. Sabemos que Cibele nasceu antes de Camila e Celeste nasceu depois de Camila.")
    st.text("Dados três números inteiros indicando as idades das irmãs, escreva um programa para determinar a idade de Camila.")

    st.subheader("Entrada")
    st.text("A entrada é composta por três linhas, cada linha contendo um número inteiro N, a idade de uma das irmãs.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo um único número inteiro, a idade de Camila.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 5 ≤ N ≤ 100")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Idade de Camila"
            dados.rota_questao = "idade_de_camila"
