...
with col1:
    ...
    with col22:
        st.header("Matriz Escada")

    st.text("Joãozinho está aprendendo sobre matrizes. Hoje ele aprendeu como deixar matrizes na forma escada, e está exercitando. Para ajudá-lo, você deve escrever um programa que determine se o resultado dele realmente está no formato correto.")
    st.text("Uma matriz está na forma escada quando, para cada linha, as condições a seguir forem satisfeitas:")
    st.markdown("&emsp; • Se a linha só possuir zeros, então todas as linhas abaixo desta também só possuem zeros.")
    st.markdown("&emsp; • Caso contrário, seja X o elemento diferente de zero mais à esquerda da linha; então, para todas as linhas abaixo da linha de X, todos os elementos nas colunas à esquerda de X e na coluna de X são iguais a zero.")

    st.subheader("Entrada")
    st.text("A primeira linha possui dois inteiros N e M, as dimensões da matriz. Cada uma das N linhas seguintes contém M inteiros não-negativos, os elementos da matriz.")

    st.subheader("Saída")
    st.text("""Seu programa deve produzir uma única linha, contendo o caractere "S" caso a matriz esteja no formato escada, ou "N", caso contrário.""")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 500 e 1 ≤ M ≤ 500.")
    st.markdown("&emsp; • Cada elemento da matriz está entre 0 e 105.")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos de teste equivalente a 80 pontos, N ≤ 50 e M ≤ 50.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Matriz Escada"
            dados.rota_questao = "matriz_escada"
