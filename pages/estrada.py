...
with col1:
    ...
    st.text("Para melhorar a integração com os países vizinhos, o Rei da Nlogônia decidiu que uma nova estrada será construída cruzando o país, da fronteira oeste à fronteira leste. O formato da estrada é uma única reta, que passará pelo centro de algumas cidades.")
    st.text("O Rei também decidiu que a construção será paga pelo Tesouro Real, mas cada cidade pela qual a estrada passar será responsável pela manutenção do trecho da estrada que constitui a vizinhança da estrada para aquela cidade. A vizinhança da estrada de uma cidade A é definida como todos os pontos da estrada que são mais próximos do centro da cidade A do que do centro de qualquer outra cidade.")
    st.text("Dados o comprimento total da estrada, de fronteira a fronteira, e as distâncias da fronteira oeste até os centros de cada cidade ao longo da nova estrada, escreva um programa para determinar qual a menor vizinhança de estrada entre as cidades pelas quais a estrada vai passar.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um inteiro T, o comprimento total da estrada. A segunda linha contém um inteiro N, o número de cidades pelas quais a estrada vai passar. Cada uma das N linhas seguintes contém um inteiro Xi, indicando a distância da fronteira oeste até o centro da cidade i. Não há cidades nas fronteiras e cada centro de cidade tem uma localização distinta.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo um número real com duas casas após o ponto decimal, a menor vizinhança de estrada entre as cidades pelas quais a estrada vai passar.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 3 ≤ T ≤ 10⁶")
    st.markdown("&emsp; • 2 ≤ N ≤ 10⁴")
    st.markdown("&emsp; • 0 < Xi < T, para 1 ≤ i ≤ N")
    st.markdown("&emsp; • Xi ≠ X, para todo par 1 ≤ i,j ≤ N.")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 10 pontos, N = 2.")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 90 pontos adicionais, nenhuma outra restrição.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Estrada"
            dados.rota_questao = "estrada"
