...
with col1:
    ...
    with col22:
        st.header("Média ou mediana")

    st.text("A média de três números inteiros A, B e C é (A + B + C)/3. A mediana de três números inteiros é o número que ficaria no meio se os três números fossem ordenados em ordem não-decrescente.")
    st.text("Sua tarefa é escrever um programa que, dados dois números inteiros distintos A e B, calcule o menor inteiro possível C tal que a média e a mediana de A, B e C sejam iguais.")
    
    st.subheader("Entrada")
    st.text("A entrada é composta de uma única linha contendo dois números inteiros A e B.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo um único número, o menor inteiro possível C tal que a média e a mediana de A, B e C são iguais.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ A ≤ B ≤ 10⁹")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Média ou Mediana"
            dados.rota_questao = "media_ou_mediana"
