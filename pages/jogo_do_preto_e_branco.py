...
with col1:
    ...
    with col22:
        st.header("Jogo do Preto e Branco")

    st.text("Você gosta de quebra-cabeças? O jogo do Preto e Branco é um quebra-cabeças que usa um tabuleiro retangular com L linhas e C colunas, formando L × C casas. No tabuleiro são posicionadas algumas peças pretas, cada peça em uma casa diferente.")
    st.text("O objetivo do jogo é colocar o maior número possível de peças brancas no tabuleiro, obedecendo às seguintes restrições:")
    st.markdown("&emsp; • cada casa do tabuleiro pode conter no máximo uma peça;")
    st.markdown("&emsp; • uma peça branca deve ter ao menos uma peça preta como vizinha, à direita, à esquerda, acima ou abaixo;")
    st.markdown("&emsp; • uma peça branca não pode ter outra peça branca como vizinha, à direita, à esquerda, acima ou abaixo;")
    st.text("A figura abaixo mostra dois exemplos de jogos, com as respectivas soluções, um com um tabuleiro 3 × 3 e outro com um tabuleiro 3 × 5.")
    
    col14, col24, col34 = st.columns([1, 1.5, 1])

    with col24:
        st.image("./images/jogo.png",width=800)

    st.text("Sua tarefa é escrever um programa que, dadas as descrições do tabuleiro e das peças pretas posicionadas, determine o maior número de peças brancas que podem ser colocadas.")

    st.subheader("Entrada")
    st.text("A primeira linha contém dois inteiros L e C, o número de linhas e o número de colunas do tabuleiro. As linhas são numeradas de 1 a L e as colunas são numeradas de 1 a C. A segunda linha contém um inteiro P, o número de peças pretas colocadas no tabuleiro. Cada uma das P linhas seguintes descreve a posição de uma peça preta e contém dois inteiros Xi e Yi, indicando a linha e a coluna em que a peça foi colocada.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo um único inteiro, o maior número de peças brancas que podem ser colocadas no tabuleiro.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ L ≤ 6")
    st.markdown("&emsp; • 1 ≤ C ≤ 6")
    st.markdown("&emsp; • 1 ≤ P ≤ 10")
    st.markdown("&emsp; • 1 ≤ Xi ≤ L para 1 ≤ i ≤ L")
    st.markdown("&emsp; • 1 ≤ Yi ≤ C para 1 ≤ i ≤ C")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 20 pontos, L = 1.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Jogo do Preto e Branco"
            dados.rota_questao = "jogo_do_preto_e_branco"
