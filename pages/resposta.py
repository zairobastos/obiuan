...
def calcular_similaridade(code1, code2):

    emb1 = model.encode([code1], convert_to_numpy=True)
    emb2 = model.encode([code2], convert_to_numpy=True)
...
def return_code(rota_questao):
    print(f"Tentando abrir o arquivo: {dados.resposta_certa[rota_questao]}")  # Debug
    with open(dados.resposta_certa[rota_questao], "r", encoding="utf-8") as arquivo:
...
with col1:
    ...
    st.header("Sua resposta está")

    st.subheader(f"""&emsp; {calcular_similaridade(return_code(dados.rota_questao), dados.resposta):.2f}% correta""")

    st.header("Questão")

    st.subheader(f"&emsp;{dados.nome_questao}")

    st.subheader("Sua resposta")

    st.code(f"{dados.resposta}", language='python')

    col13, col23, col33 = st.columns([4, 0.65, 0.45])
    
    with col23:
        if st.button("Tentar novamente", key="novamente"):
            st.switch_page(f'pages/{dados.rota_questao}.py')
    with col33:
        if st.button("Pedir dicas", key="dicas"):
            st.switch_page(f'pages/dicas.py')
