...
with col1:
    ...
    with col22:
        st.header("Macacos me Mordam!")

    st.text("Em uma floresta há N árvores alinhadas. A i-ésima árvore tem altura Hi e está localizada na posição Xi da floresta. Obi, o macaco camarada, está na primeira árvore da floresta, e deseja ir até a última árvore da floresta, porque ele ouviu dizer que há muitas bananas esperando por ele lá.")
    ...
    with col24:
        st.image("./images/macacos.png",width=500)

    st.text("Na figura acima podemos ver que, do topo da árvore na posicão 2, Obi não consegue enxergar o topo da árvore na posição 4, e portanto ele não pode pular de uma para outra sem passar pela árvore na posição 3. Assim, para o caso da figura acima, para ir da árvore 1 para a árvore 4 ele tem que passar por todas as árvores, dando um total de três pulos.")
    st.text("Dada a descrição da floresta, você deve escrever um programa para determinar o menor número de pulos que Obi deve dar para ir da primeira à última árvore da floresta.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um número N, indicando a quantidade de árvores na floresta. Cada uma das N linhas seguintes descreve uma árvore da floresta, e contém dois inteiros Xi e Hi, respectivamente a posição e a altura de uma árvore. Cada árvore ocupa uma posição distinta na floresta (ou seja, não há duas árvores com o mesmo valor Xi).")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo um único número inteiro, a menor a quantidade de pulos que Obi deve dar para ir da primeira até a última árvore da floresta.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 2 ≤ N ≤ 3 x 10⁵")
    st.markdown("&emsp; • 1 ≤ Hi, Xi ≤ 10⁹")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos de teste cuja soma é 40 pontos: 2 ≤ N ≤ 300")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Macacos me Mordam"
            dados.rota_questao = "macacos_me_mordam"
