...
with col1:
    ...
    st.text("Um movimento consiste em retirar uma peça preta criando um espaço e inverter as peças vizinhas à retirada. Sendo assim, dependendo do número de peças vizinhas à retirada, um movimento pode inverter duas, uma, ou mesmo nenhuma peça (se não houver peças vizinhas à que está sendo retirada). Você vence o jogo quando consegue remover todas as peças. A figura abaixo exemplifica uma sequência de movimentos que resolvem uma instância do problema com 5 peças, em que as peças são retiradas na ordem 5-2-1-3-4.")
    
    col15, col25, col35 = st.columns([1, 1, 1])

    with col25:
        st.image("./images/vira3.png",width=500)

    st.text("Para uma determinada disposição inicial das peças, podem existir várias soluções diferentes. Por exemplo, poderíamos retirar as peças na ordem 5-2-3-4-1 e ainda assim conseguir retirar todas as peças.")
    st.text("Sua tarefa, neste problema, consiste em contar o número de soluções diferentes para uma dada disposição inicial das peças. Como o número de soluções pode ser muito grande, você deve imprimir apenas o resto do número quando dividido por 10007.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém o inteiro N. A linha seguinte contém N letras separadas por espaço representando o arranjo inicial das peças. Uma peça branca é indicada pela letra B na entrada, e uma peça preta é indicada pela letra P.")

    st.subheader("Saída")
    st.text("Seu programa deve imprimir uma linha contendo o número de soluções distintas que resolvem o jogo.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 1000.")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Em um conjunto de casos de teste que totaliza 30 pontos, N ≤ 7.")
    st.markdown("&emsp; • Em um conjunto de casos de teste totalizando 60 pontos, N ≤ 1000.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Vira!"
            dados.rota_questao = "vira"
