...
with col1:
    ...
    with col22:
        st.header("Rede social")

    st.text("Uma nova rede social foi lançada e fez sucesso imediato. Nessa nova rede é possível postar mensagens que são recebidas por seguidores; um seguidor pode decidir repostar uma mensagem que recebeu e seus seguidores também receberão a mensagem e poderão por sua vez repostá-la.")
    st.text("Para medir a influência de um usuário na nova rede foi criado um novo critério, chamado de Fator de Influência, descrito a seguir.")
    st.markdown("&emsp; • Inicialmente vamos definir o índice de repostagem de uma mensagem M de um usuário U como sendo o número de usuários diferentes de U que repostaram M.")
    st.markdown("&emsp; • O Fator de Influência de um usuário U é o máximo valor de FI tal que U postou FI mensagens que, cada uma, tem um índice de repostagem de pelo menos FI.")
    st.text("Por exemplo, se João postou quatro mensagens, com índices de repostagem 1, 1, 5, 6, seu Fator de Influência é 2, pois postou duas mensagens com índice de repostagem maior ou igual a 2.")
    ...
    st.text("A primeira linha da entrada contém um inteiro N, o número de mensagens postadas pelo usuário. Cada uma das N linhas seguintes contém um inteiro Ri, o índice de repostagem de uma mensagem.")

    st.subheader("Saída")
    st.text("Seu programa deve produzir uma única linha, contendo um único número inteiro, o Fator de Influência para o usuário.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 5 × 10⁵")
    st.markdown("&emsp; • 0 ≤ Ri ≤ 10⁶ para 1 ≤ i ≤ N")

    st.subheader("Informações sobre a pontuação")
    st.markdown("&emsp; • Para um conjunto de casos de testes valendo 20 pontos, 1 ≤ N ≤ 1000.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Rede social"
            dados.rota_questao = "rede_social"
