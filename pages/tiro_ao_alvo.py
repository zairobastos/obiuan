...
with col1:
    ...
    with col22:
        st.header("Tiro ao alvo")

    st.text("Recentemente Juquinha ganhou de aniversário um joguinho bem clássico: Tiro ao Alvo. Ele arrumou um ótimo lugar em seu quarto para se divertir com o jogo, porém após ler todas as regras do jogo ele percebeu que precisa da sua ajuda para calcular a pontuação obtida.")
    st.text("Segundo as regras, o alvo do jogo é composto por C círculos, todos centrados na origem (0,0). Juquinha atira T vezes e após cada tiro informa suas coordenadas. A pontuação de cada tiro é feita da seguinte forma: para cada círculo em que o tiro estiver contido Juquinha recebe um ponto.")
    st.text("Considere por exemplo a figura abaixo. O tiro marcado com a letra A recebe zero pontos, pois não está contido por nenhum círculo. O tiro marcado com a letra B recebe um ponto, pois está contido por um círculo (o mais externo). O tiro marcado com a letra \sf C recebe dois pontos, pois está contido por dois círculos (note que este caso mostra que tiros exatamente na borda de um círculo são considerados como contidos pelo círculo). Já o tiro marcado com a letra D recebe três pontos, pois está contido pelos três círculos. Considerando todos os pontos, a pontuação total de Juquinha é de 13 pontos.")
    
    st.image("./images/tiro.png",width=300)

    st.text("Dados os raios de C círculos centrados na origem e as coordenadas dos T tiros realizados por Juquinha, escreva um programa que calcula o total de pontos que Juquinha obteve.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém dois inteiros positivos, C e T, que representam, respectivamente, o número de círculos do alvo e o número de tiros.")
    st.text("Cada uma das C linhas seguintes contém um inteiro positivo. O i-ésimo inteiro R_i representa o raio do i-ésimo círculo. Os raios R_i são fornecidos em ordem crescente. Cada uma das T linhas seguintes contém um par X,Y de inteiros, que representam as coordenadas de cada tiro.")

    st.subheader("Saída")
    st.text("Seu programa deve imprimir uma única linha, contendo apenas um inteiro, o total de pontos obtidos por Juquinha.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ C ≤ 10⁵")
    st.markdown("&emsp; • 1 ≤ R_i ≤ 10⁶ para 1 ≤ i ≤ C")
    st.markdown("&emsp; • R_i > R_i-1 para 2 ≤ i ≤ C")
    st.markdown("&emsp; • 1 ≤ T ≤ 10⁵")
    st.markdown("&emsp; • -10⁵ ≤ X,Y ≤ 10⁵") 

    st.subheader("Informações sobre a pontuação")
    st.text("Em um conjunto de casos de teste que totaliza 30 pontos:")
    st.markdown("&emsp; • 1 ≤ C ≤ 10³")
    st.markdown("&emsp; • 1 ≤ R_i ≤ 10⁴ para 1 ≤ i ≤ N")
    st.markdown("&emsp; • 1 ≤ T ≤ 10³")
    st.markdown("&emsp; • -10² ≤ X,Y ≤ 10²")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Tiro ao Alvo"
            dados.rota_questao = "tiro_ao_alvo"
