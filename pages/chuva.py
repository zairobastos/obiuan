...
with col1:
    ...
    st.text("Está chovendo tanto na Obilândia que começaram a aparecer goteiras dentro da casa do prefeito. Uma dessas goteiras está fazendo escorrer água verticalmente, a partir de um ponto no teto, numa parede onde há várias prateleiras horizontais. Quando a água bate em uma prateleira, ela começa a escorrer horizontalmente para os dois lados, direita e esquerda, até as extremidades da prateleira, quando volta a escorrer verticalmente.")
    st.text("""Vamos representar a parede por uma matriz de N linhas e M colunas de caracteres, como mostrado abaixo. As prateleiras serão representadas por "#" e a parede por ".". Só existem prateleiras nas linhas pares e elas nunca encostam na borda da parede. Há apenas um ponto de vazamento representado pelo caractere " o" na primeira linha.""")
    
    col14, col24, col34 = st.columns([1.4, 1, 1])

    with col24:
        st.image("./images/chuva1.png",width=150)

    st.text("""Para deixar mais rigorosa a forma como a água vai escorrer, seja c(i,j) o caractere na linha i coluna j. Se c(i,j)= ".", então ele deve virar "o" sempre que:""")
    st.markdown("""&emsp; • c(i-1,j)= "o"; ou""")
    st.markdown("""&emsp; • c(i,j-1)= "o" e c(i+1,j-1)= "#"; ou""")
    st.markdown("""&emsp; • c(i,j+1)= "o" e c(i+1,j+1)= "#".""")
    st.text("Dada uma lista com os índices de repostagens de todas as mensagens postadas por um usuário, escreva um programa para calcular o Fator de Influência do usuário.")

    col15, col25, col35 = st.columns([1.4, 1, 1])

    with col25:
        st.image("./images/chuva2.png",width=150)

    st.text("""Neste problema, dada a matriz representando a parede no início do vazamento, seu programa deve imprimir na saída uma matriz representando a parede usando o caractere "o" exatamente nas posições que serão molhadas pelo vazamento, como ilustrado acima.""")

    st.subheader("Entrada")
    st.text("""A primeira linha da entrada contém dois inteiros N e M, respectivamente o número de linhas e colunas da matriz. As N linhas seguintes da entrada contêm, cada uma, uma sequência de M caracteres entre três possíveis: ".", "#" ou "o".""")

    st.subheader("Saída")
    st.text("""Seu programa deve imprimir N linhas, cada uma contendo uma sequência de M caracteres, representando a matriz da entrada usando o caractere "o" exatamente nas posições que serão molhadas pelo vazamento.""")

    st.subheader("Restrições")
    st.markdown("&emsp; • 3 ≤ N ≤ 500 e 3 ≤ M ≤ 500;")
    st.markdown("&emsp; • O número de linhas N é ímpar;")
    st.markdown("""&emsp; • Há exatamente um caractere "o" na primeira linha;""")
    st.markdown("""&emsp; • As linhas ímpares, a primeira coluna e a última coluna não possuem o caractere "#".""")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Chuva"
            dados.rota_questao = "chuva"
