...
with col1:
    ...
    with col22:
        st.header("Calçada Imperial")

    st.text("Na calçada em frente ao Palácio Imperial, não se sabe a razão, existe uma sequência de N números desenhados no chão. A sequência é composta apenas pelos números de 1 a N. Veja um exemplo na coluna (a) da figura abaixo, para N=12.")
    
    col14, col24, col34 = st.columns([1.4, 1, 1])

    with col24:
        st.image("./images/calcada.png",width=150)

    st.text("Ninguém sabe o significado da sequência e, justamente por isso, várias teorias malucas surgiram. Uma delas diz que a sequência representa, na verdade, apenas um valor que estaria relacionado a um grande segredo dos imperadores. Esse valor é a quantidade máxima de números da sequência que poderiam ser marcados com um círculo, de modo que a sequência de números marcados não contenha dois números iguais consecutivos e seja composta de no máximo dois números distintos.")
    st.text("A coluna (b) da figura ilustra uma sequência de 4 números marcados que obedece a restrição acima. Você consegue verificar que essa é, de fato, a quantidade máxima possível de números numa sequência marcada?")
    ...
    st.text("A primeira linha da entrada contém um inteiro N representando o tamanho da sequência. As N linhas seguintes contêm, cada uma, um inteiro Vi, para 1 ≤ i ≤ N, definindo a sequência de números desenhados no chão da calçada imperial.")

    st.subheader("Saída")
    st.text("Seu programa deve imprimir uma linha contendo um número inteiro representando a quantidade máxima de números da sequência que poderiam ser marcados com um círculo sem que haja dois números iguais consecutivos na sequência marcada e tal que ela seja composta de no máximo dois números distintos.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 1 ≤ N ≤ 500")
    st.markdown("&emsp; • 1 ≤ Vi ≤ N, para 1 ≤ i ≤ N")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Calçada Imperial"
            dados.rota_questao = "calcada_imperial"
