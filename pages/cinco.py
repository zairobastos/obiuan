...
with col1:
    ...
    st.text("Considere um número decimal não divisível por 5. Queremos fazer exatamente uma operação de troca entre os dígitos de duas posições distintas para obter um número que seja divisível por 5. Quer dizer, precisamos escolher duas posições distintas e trocar os dígitos dessas duas posições. Mas queremos que o número resultante após a troca seja o maior número divisível por 5 possível.")
    
    col14, col24, col34 = st.columns([1.1, 1, 1])

    with col24:
        st.image("./images/cinco.png",width=300)

    st.text("Veja o exemplo da figura, 730105697542, que não é divisível por 5. Podemos fazer a primeira troca ilustrada e obter 730102697545, que é divisível por 5. Mas, se fizermos a segunda troca ilustrada na figura, vamos obter um número divisível por 5 ainda maior, 732105697540.")
    st.text("Dados os dígitos decimais de um número na entrada, não divisível por 5, seu programa deve imprimir os dígitos decimais do maior número divisível por 5 que pode ser obtido com exatamente uma troca de dígitos entre duas posições distintas. Caso não seja possível obter um número divisível por 5, imprima apenas -1.")

    st.subheader("Entrada")
    st.text("A primeira linha da entrada contém um inteiro N, indicando quantos dígitos decimais tem o número não divisível por 5. A segunda linha contém N inteiros Di, 1 ≤ i ≤ N, representando os dígitos decimais do número em questão.")

    st.subheader("Saída")
    st.text("Imprima uma linha contendo N inteiros representando os dígitos decimais do maior número divisível por 5 que pode ser obtido com exatamente uma troca de dígitos entre duas posições distintas. Caso não seja possível obter um número divisível por 5, imprima apenas -1.")

    st.subheader("Restrições")
    st.markdown("&emsp; • 2 ≤ N ≤ 1000")
    st.markdown("&emsp; • Di é um inteiro entre 0 e 9, inclusive.")
    ...
    with col23:
        if st.button("Pedir dicas", key="dicas"):
            ...
            dados.nome_questao = "Cinco"
            dados.rota_questao = "cinco"
