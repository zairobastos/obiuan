...
with st.sidebar:
    ...
    st.title("Módulos - Áreas Temáticas")
...
with col1:
    # Conteúdo principal
    st.title("👋 Seja bem vindo a OBI-UAN 🤖")

    st.subheader("O que é a OBI?")
    st.text("A Olimpíada Brasileira de Informática (OBI) é uma competição nacional que desempenha papel relevante na formação de estudantes da área de Tecnologia da Informação, ao incentivá-los a desenvolver habilidades em matemática e teoria da computação por meio da resolução de problemas.")
    st.text("Seu principal objetivo é despertar nos alunos o interesse por uma ciência importante na formação básica hoje em dia (no caso, ciência da computação), através de uma atividade que envolve desafio, engenhosidade e uma saudável dose de competição.")

    st.subheader("Nossa ferramenta")
    st.text("A OBI-UAN (Um Agente Natural da OBI) usa IA para ajudar você a estudar para a OBI, oferecendo planos de estudo, classificação de questões e dicas personalizadas com base nas suas respostas.")
    st.text("Escolha as questões que deseja estudar usando os menus suspensos ao lado. É só selecionar o módulo e começar!")
